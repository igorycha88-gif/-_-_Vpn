...
func (r *sqlitePeerRepository) Create(ctx context.Context, p *models.Peer) error {
	q := `INSERT INTO wg_peers (id, name, email, device_type, config_mode, public_key, private_key, address, dns, mtu, is_active)
	      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
	_, err := r.db.ExecContext(ctx, q,
		p.ID, p.Name, p.Email, p.DeviceType, p.ConfigMode, p.PublicKey, p.PrivateKey,
...
func (r *sqlitePeerRepository) GetByID(ctx context.Context, id string) (*models.Peer, error) {
	q := `SELECT id, name, email, device_type, config_mode, public_key, private_key, address, dns, mtu,
...
func (r *sqlitePeerRepository) List(ctx context.Context) ([]*models.Peer, error) {
	q := `SELECT id, name, email, device_type, config_mode, public_key, '', address, dns, mtu,
	             is_active, created_at, updated_at, total_rx, total_tx, last_seen
...
func (r *sqlitePeerRepository) Update(ctx context.Context, p *models.Peer) error {
	q := `UPDATE wg_peers SET name=?, email=?, device_type=?, config_mode=?, dns=?, mtu=?, is_active=?, updated_at=CURRENT_TIMESTAMP
	      WHERE id=?`
	result, err := r.db.ExecContext(ctx, q, p.Name, p.Email, p.DeviceType, p.ConfigMode, p.DNS, p.MTU, p.IsActive, p.ID)
	if err != nil {
