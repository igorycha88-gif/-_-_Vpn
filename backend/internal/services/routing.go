...
func NewRoutingService(routeRepo repository.RouteRepository, logger *slog.Logger) *RoutingService {
...
func (s *RoutingService) GetRule(ctx context.Context, id string) (*models.RoutingRule, error) {
	...
	if err != nil {
		return nil, s.mapErr(fmt.Errorf("service.routing.GetRule: %w", err))
	}
	return rule, nil
}
...
func (s *RoutingService) UpdateRule(ctx context.Context, id string, req *models.RoutingRuleUpdateRequest) (*models.RoutingRule, error) {
	rule, err := s.routeRepo.GetByID(ctx, id)
	if err != nil {
		return nil, s.mapErr(fmt.Errorf("service.routing.UpdateRule get: %w", err))
	}
	...
	if req.Type != nil {
		if !models.ContainsString(models.ValidRuleTypes, *req.Type) {
	...
	if req.Action != nil {
		if !models.ContainsString(models.ValidRuleActions, *req.Action) {
...
func (s *RoutingService) DeleteRule(ctx context.Context, id string) error {
	if err := s.routeRepo.Delete(ctx, id); err != nil {
		return s.mapErr(fmt.Errorf("service.routing.DeleteRule: %w", err))
	}
...
func (s *RoutingService) mapErr(err error) error {
	if err == nil {
		return nil
	}
