...
└──────────────┘                                                 ▼
                         ...
                         │  ┌──────────────┐    ┌────────────────────────┐   │
                         │  │  WireGuard   │───▶│  sing-box              │   │
                         │  │  Server      │    │  SOCKS :1080 (wg0)     │   │
                         │  │  (wg0 :51821)│    │  + DNS resolver        │   │
                         │  └──────────────┘    │  + NAT/Masquerade      │   │
                         │                      │  → Global Internet     │   │
                         │                      └────────────────────────┘   │
                         │                                                   │
...
- **WireGuard сервер** на порту 51821/UDP (интерфейс `wg0`) — приём тоннеля от РФ-сервера
- **sing-box** — SOCKS-прокси и DNS-резолвер:
  - SOCKS5 прокси на `10.20.0.2:1080` (принимает трафик от РФ-сервера через WG тоннель)
  - DNS-резолвер (эфемерные UDP-порты) для разрешения доменов через зарубежные DNS
  - Исходящий трафик → напрямую в интернет
- NAT через iptables:
  ```bash
...
```
РФ-сервер (sing-box WG outbound)
    │
    ▼ WG тоннель (10.20.0.0/30, порт 51821/UDP)
Зарубежный сервер
    │
    ├─ wg0 → 10.20.0.2
    │
    ▼
sing-box SOCKS5 (10.20.0.2:1080)
    │
    ├─ DNS-резолв через зарубежные DNS (1.1.1.1, 8.8.8.8)
    │
    ▼
Прямое подключение к глобальному интернету
```
...
| **Нет открытых портов** | RU: :80/:443 (HTTPS/VLESS), :51820/:51821 (WG). Foreign: :51821/UDP (WG), :1080 (SOCKS, только wg0), :443 (sing-box) |
...
| **80** | TCP | nginx (docker-proxy) | HTTP → редирект на HTTPS |
| **443** | TCP | sing-box | VLESS+Reality (клиентские подключения) |
| **8080** | TCP | api (Go) | Backend REST API |
| **9090** | TCP | sing-box | Clash API (только localhost, мониторинг) |
| **51820** | UDP | WireGuard | Клиентский VPN (wg0) |
...
|---|---|---|---|
| **22** | TCP | sshd | SSH-доступ |
| **53** | TCP/UDP | systemd-resolve | Локальный DNS (127.0.0.53/54, loopback only) |
| **443** | TCP | sing-box | Входящий HTTPS (маскировка) |
| **1080** | TCP | sing-box | SOCKS5-прокси (только 10.20.0.2, через WG тоннель) |
| **51821** | UDP | WireGuard | Приём межсерверного тоннеля от РФ-сервера (wg0) |
| **53983, 60225, 32846, 46185** | UDP | sing-box | Эфемерные порты DNS-резолвера |

---

## 8. Docker Compose — деплой
