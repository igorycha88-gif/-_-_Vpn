...
- WireGuard VPN для подключения клиентов
...
- Управление WireGuard-клиентами (добавление/удаление, QR-коды)
...
│              │         │                                                    │
│   Клиенты    │  WG     │  ┌──────────┐    ┌─────────────────────────────┐  │
│  (телефон,   │ tunnel  │  │WireGuard │───▶│       Routing Engine        │  │
│   ПК,        │────────▶│  │  Server  │    │       (sing-box)            │  │
│   планшет)   │  :51820 │  │  (wg0)   │    │                             │  │
│              │         │  └──────────┘    │  ┌─────────┐ ┌──────────┐  │  │
└──────────────┘         │                  │  │ Direct  │ │  Foreign  │  │  │
...
### 3.1. WireGuard VPN (РФ-сервер)

- **Порт**: 51820/UDP
- **Интерфейс**: `wg0`
- **Назначение**: приём клиентских подключений
- **IP-пул клиентов**: 10.10.0.0/24
- **Конфигурация**: генерируется через API админ-панели
...
Располагается на РФ-сервере, работает как transparent proxy:

```
Клиентский трафик (wg0)
    │
    ▼
iptables REDIRECT (TCP:  → sing-box :12345)
                (UDP DNS → sing-box :5353)
    │
    ▼
...
```json
{
  "outbounds": [
    ...
    },
    {
      "type": "wireguard",
      "tag": "foreign-out",
      "server": "<foreign-server-ip>",
      "server_port": 51821,
      "local_address": ["10.20.0.2/30"],
      "private_key": "<key>",
      "peer_public_key": "<key>",
      "reserved": [0, 0, 0],
      "mtu": 1280
...
# WireGuard клиенты
GET    /api/v1/wg/peers
POST   /api/v1/wg/peers              # Добавить клиента
DELETE /api/v1/wg/peers/{id}         # Удалить клиента
GET    /api/v1/wg/peers/{id}/config  # Скачать .conf
GET    /api/v1/wg/peers/{id}/qr      # QR-код для подключения
...
| **WireGuard → Клиенты** | Список клиентов, добавление/удаление, генерация конфигов и QR-кодов |
...
```
    ...
    │                                     │
    │  eth0: <public-ip>                  │
    │  wg0:  10.10.0.1/24 (клиенты)      │
    │  wg1:  10.20.0.1/30 → зарубежный   │
    │                                     │
    │  iptables:                          │
    │    wg0 → REDIRECT → sing-box        │
    │    sing-box → direct ИЛИ wg1        │
    │                                     │
    │  Nginx :443 → Landing + Admin       │
    │  WG    :51820 → VPN клиенты         │
    └──────────────────┬──────────────────┘
                       │ WG tunnel (wg1)
                       │ 10.20.0.0/30
    ┌──────────────────┴──────────────────┐
...
# NAT для клиентов WireGuard (рунет)
iptables -t nat -A POSTROUTING -s 10.10.0.0/24 -o eth0 -j MASQUERADE

# Перенаправление клиентского TCP трафика в sing-box (transparent proxy)
iptables -t nat -A PREROUTING -i wg0 -p tcp -j REDIRECT --to-port 12345

# Перенаправление DNS запросов клиентов в sing-box DNS
...
| **Порт WG нестандартный** | WireGuard на нестандартном порту (51820 не виден из браузера) |
| **API за аутентификацией** | Админка и API доступны только по JWT-токену |
| **Rate Limiting** | Защита от брутфорса на /api/v1/auth/login |
| **Нет открытых портов** | Только :443 (HTTPS) и :51820 (UDP WireGuard) |
...
│   │   │   ├── peers.go             # Управление WG клиентами
...
│   │   ├── services/
│   │   │   ├── wireguard.go         # Логика управления WireGuard
│   │   │   ├── singbox.go           # Генерация конфигов sing-box
...
│   └── pkg/
│       ├── wgcrypto/
│       │   └── keys.go              # Генерация ключей WireGuard
│       └── qrcode/
...
│       │   ├── Login.tsx
│       │   ├── Dashboard.tsx
│       │   ├── Peers.tsx            # Управление клиентами WG
...
### v1.0 — MVP
- [x] WireGuard VPN (клиентский + межсерверный)
- [x] sing-box с базовой маршрутизацией (рунет / мир)
...
- [ ] Поддержка IPv6
- [ ] WireGuard-over-WS/WS-TLS для обхода DPI блокировок WG
